...
**O proxy espera com prazo (09/09/2026).** Os dois proxies same-origin (`/api/auth/*` e `/api/v1/*`) passaram a compartilhar `apps/app/src/lib/api-proxy.ts`, e toda leitura de `apps/api` a partir de `apps/app` — proxy, `apiFetch` dos Server Components e a leitura de flags — corre com `AbortSignal.timeout` (`lib/upstream-timeout.ts`, 15 s). Sem isso uma API que aceitava a conexão e não respondia deixava o browser esperando pelo tempo que o runtime permitisse: botão de login em "Verificando…" e shell do painel em "verificando sessão", sem nada a que reagir. Estourado o prazo (ou API inalcançável), o proxy responde no envelope do projeto (§5.7) com `504`/`upstream.timeout` ou `502`/`upstream.unreachable` — os únicos dois `reason` que nascem em `apps/app`, não em `apps/api`. Do lado do formulário de login, `fetch` que rejeita ou que volta sem ser 401/403 vira o banner de conexão (`backoffice.connection_error`), nunca o de credenciais: mandar alguém redigitar uma senha que a plataforma nem chegou a conferir era o bug de 09/09.
